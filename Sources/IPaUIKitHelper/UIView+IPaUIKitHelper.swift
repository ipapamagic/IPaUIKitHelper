...
extension UIView {
    ...
    public func roundCorners(corners: UIRectCorner, radius: CGFloat) {
        ...
        objc_setAssociatedObject(self, &roundCornerHandle, corners.rawValue, objc_AssociationPolicy.OBJC_ASSOCIATION_ASSIGN)
    ...
    func addSizeObserver() {
        ...
        let sizeObserver = self.observe(\.center) { view, value in
            if let cornersInt = objc_getAssociatedObject(self, &roundCornerHandle) as? UInt,let radius = objc_getAssociatedObject(self, &roundCornerRadiusHandle) as? CGFloat {
                let corners = UIRectCorner(rawValue: cornersInt) 
...
//        borderShapeLayer.bounds = self.layer.bounds
    ...
    public func addSubviewAlignSafeArea(_ view:UIView,edgeInsects:UIEdgeInsets) {
        ...
        view.bottomAnchor.constraint(equalTo: self.safeAreaLayoutGuide.bottomAnchor, constant: -edgeInsects.bottom).isActive = true
